(function () {
  ...
  const liAll = document.querySelector('.nav-tabs').firstElementChild;
  const liCompleted = document.querySelector('.nav-tabs').lastElementChild;
  ...
  liAll.addEventListener('click', showAllTasks);
  liCompleted.addEventListener("click", showCompletedTasks);
  ...
  function showCompletedTasks() {
    const taskCompleted = Object.values(objOfTasks)
      .filter(task => task.completed === true)
      .reduce((acc, task) => { acc[task._id] = task; return acc; }, {})
    listContainer.innerHTML = '';
    renderAllTasks(taskCompleted);
  }
